...
Tien * ReadFromFile(int *n){
	...
	while (!feof(f)){
		...
		dslt[i].PA=0;
		i++;
		dslt=(Tien*)realloc(dslt,
				sizeof(Tien)*(i+1)); 
	}
	*n=i;
	fclose(f); return dslt;
}
...
// Sap xep theo thu tu tang cua Menh gia tien
...
void InDS(Tien *dslt ,int n, int TienCanRut){
	...
	printf("|---|-------------------------|---------|---------|----------|\n");
			...
			printf("|---|-------------------------|---------|---------|----------|\n");
					for(i=0;i<n;i++){
						printf("|%-3d",i+1);printf("|%-25s",dslt[i].TenTien);
						printf("|%-9d",dslt[i].MG);
						printf("|%-9d",dslt[i].PA);
						printf("|%-10d|\n",dslt[i].MG*dslt[i].PA);
						TongTienTra=TongTienTra+dslt[i].PA *
								dslt[i].MG;
						}
						printf("|---|-------------------------|---------|---------|----------|\n");
...
void Greedy(Tien *dslt,int n, int TienCanRut){
	...
	while(i<n&&TienCanRut>0) {
		...
		TienCanRut = TienCanRut-dslt[i].PA *
				dslt[i].MG;
		i++;
	}
}
int main(){
	...
	dslt=ReadFromFile(&n);
	BubbleSort(dslt,n);
	Greedy(dslt,n,TienCanRut);InDS(dslt,n,TienCanRut);
	free(dslt);
	return 0;
}
